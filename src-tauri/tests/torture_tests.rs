...
async fn test_transparency_preservation() {
    ...
    assert!(
        ...
        "Failed to create transparent PNG: {}",
    ...
    let mut magick_probe = get_binary_command(handle, "magick").await.unwrap();
    ...
    let mean_str = String::from_utf8_lossy(&probe_output.stdout)
        .trim()
        .to_string();
    let mean: f64 = mean_str.parse().unwrap_or(0.0);
    assert!(
        mean > 1000.0,
        "Output JPG appears to be a black block (mean: {})",
        mean
    );
}
...
async fn test_ffmpeg_metadata_preservation() {
    ...
    let mut ffmpeg_cmd = get_binary_command(handle, "ffmpeg").await.unwrap();
    let output = ffmpeg_cmd
        .args([
            ...
            "-metadata",
            "title=TortureTestTitle",
            "-metadata",
            "artist=File2FileTester",
            input_path.to_str().unwrap(),
        ])
        .output()
        .await
    ...
    assert!(
        output.status.success(),
        "Failed to create metadata video: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    let mut ffprobe_cmd = get_binary_command(handle, "ffprobe").await.unwrap();
    ...
    assert!(
        probe_str.contains("TortureTestTitle"),
        "Title metadata lost. Probe output: {}",
        probe_str
    );
    assert!(
        probe_str.contains("File2FileTester"),
        "Artist metadata lost. Probe output: {}",
        probe_str
    );
}
...
async fn test_pandoc_math_preservation() {
    ...
    assert!(
        html_content.contains("math inline")
            || html_content.contains("math display")
            || html_content.contains("x^2"),
